// chapter27-bouncingpointcloud.js - Animation cho chương 27: Bouncing Point Cloud (Canvas)
// Được chuyển từ raw_animation/27, tuân thủ chuẩn animation module của hệ thống
import animationUtils from './animation-utils.js';

const chapter27BouncingPointCloud = {
  settings: {
  ...
  init(container) {
    ...
    const width = self.settings.width;
    const height = self.settings.height;
    // Setup canvas
    ...
    const ball = {
      x: width / 2,
      y: height / 2,
      radius: self.settings.ballRadius,
      vx: (Math.random() * 2 - 1) * 2,
      vy: (Math.random() * 2 - 1) * 2
    };
    // Particle system
    const numPoints = self.settings.numPoints;
    const points = [];
    let time = 0;
    ...
    let lastFrameTime = 0;
    const targetFPS = self.settings.targetFPS;
    ...
    function animate(currentTime) {
      ...
      if (deltaTime >= frameInterval) {
        ...
        ball.x += ball.vx;
        ball.y += ball.vy;
        // Bounce off edges with randomness
        ...
        const minSpeed = self.settings.minSpeed;
        const maxSpeed = self.settings.maxSpeed;
...
export default chapter27BouncingPointCloud;
